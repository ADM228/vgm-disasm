; initialization
0840: 20        clrp
0841: e8 00     mov   a,#$00    ;\ _
0843: c4 f4     mov   $f4,a     ; |
0845: c4 f5     mov   $f5,a     ; | Clear Ports
0847: c4 f6     mov   $f6,a     ; |_
0849: c4 f7     mov   $f7,a     ;/
084b: c4 83     mov   $83,a     ;>- Clear $83 (?) 
084d: 8f 30 f1  mov   $f1,#$30  ;
0850: cd ff     mov   x,#$ff    ;\_ Reset the stack
0852: bd        mov   sp,x      ;/
0853: 3f 00 40  call  $4000     ;>- Set up a lot
...
1219: b3 e3 03  bbc5  $e3,$121f   ; | if $80.1 is clear or $80.3 is clear or $e3.5 is set:
121c: 48 ff     eor   a,#$ff      ; |     VOL(R) = (!VOL(R) + 1) 
121e: bc        inc   a           ; |_
121f: 2d        push  a           ;/
...
1356: db $28,$08,$20,$10,$18,$18,$10,$20,$08,$28,$10,$20,$18,$18,$20,$10,$7f

1368: 8f 00 94  mov   $94,#$00
136b: e4 9f     mov   a,$9f       ;\ _
136d: f0 0c     beq   $137b       ; |
136f: 23 80 07  bbs1  $80,$1379   ; |
1372: 73 80 06  bbc3  $80,$137b   ; |
1375: 68 05     cmp   a,#$05      ; |
1377: b0 02     bcs   $137b       ; | if $009f > #$05 or $0080.1 == 1:
1379: e8 01     mov   a,#$01      ; |     y = #$02
137b: 1c        asl   a           ; | else:
137c: fd        mov   y,a         ; |     y = $009f * 2
137d: f6 8e 13  mov   a,$138e+y   ; | Select Echo Volumes from the table @ $138e using y
1380: 8f 2c f2  mov   $f2,#$2c    ; |
1383: c4 f3     mov   $f3,a       ; |
1385: f6 8f 13  mov   a,$138f+y   ; |
1388: 8f 3c f2  mov   $f2,#$3c    ; |_
138b: c4 f3     mov   $f3,a       ;/
138d: 6f        ret
...
13a0: e4 93     mov   a,$93       ;\ _
13a2: 1c        asl   a           ; |
13a3: 1c        asl   a           ; |
13a4: 1c        asl   a           ; |
13a5: fd        mov   y,a         ; | Set Echo filters according to
13a6: fa e1 f2  mov   $f2,$e1     ; | one of the 4 tables below, 
13a9: f6 b7 13  mov   a,$13b7+y   ; | depending on the value of $0093
13ac: c4 f3     mov   $f3,a       ; | (#$00..#$03)
13ae: 60        clrc              ; |
...
2310: e4 95     mov   a,$95       ;a = $95
2312: 3f 26 23  call  $2326
2315: c4 f5     mov   $f5,a
2317: e4 96     mov   a,$96       ;a = $96
2319: 3f 26 23  call  $2326
231c: c4 f6     mov   $f6,a
231e: e4 97     mov   a,$97       ;a = $97
2320: 3f 26 23  call  $2326
2323: c4 f7     mov   $f7,a
2325: 6f        ret

2326: f0 3f     beq   $2367       ;\ _
2328: c4 e8     mov   $e8,a       ; |
232a: 68 10     cmp   a,#$10      ; |
232c: b0 3a     bcs   $2368       ; |
232e: 5d        mov   x,a         ; |
...
2346: 6f        ret               ; | If a == 0:
                                  ; |     return
2347: 78 04 e8  cmp   $e8,#$04    ; | $e8 = a
234a: d0 0c     bne   $2358       ; | if a > #$10 and a <= #$40:
234c: e4 9e     mov   a,$9e       ; |     a = $01a0+x ($01b1..$01e0)
234e: f0 07     beq   $2357       ; | else if a > #$10 and a > #$40:
2350: 9c        dec   a           ; |     $ef = #$02
2351: f8 ce     mov   x,$ce       ; |     $e9 = $e8 && #%00001111 (#$00..#$0f)
2353: 8d 00     mov   y,#$00      ; |     $ee = $e9 << 5 (aka $9 * 32) (#$00,#$20,#$40..#$e0)
2355: 9e        div   ya,x        ; |     a = $e8
2356: bc        inc   a           ; |     if $e8 <= 50:
2357: 6f        ret               ; |          a = (($00ef)($00ee)+#$07)
                                  ; | else if ($238f+x) ($2390..$239e) != 0:
2358: 78 0c e8  cmp   $e8,#$0c    ; |     x = a
235b: d0 03     bne   $2360       ; |     a = (a)
235d: 42 81     set2  $81         ; | else if $e8 == #$03:
235f: 6f        ret               ; |     a = $0205
                                  ; | else if $00e8 == #$04 and $009e != #$00:
2360: 78 0d e8  cmp   $e8,#$0d    ; |     ya = ($009e - 1) / $00ce
2363: d0 02     bne   $2367       ; |     a += 1
2365: 52 81     clr2  $81         ; | else if $00e8 == #$0c:
2367: 6f        ret               ; |     $0081.2 = 1
                                  ; | else if $00e8 == #$0d:
2368: 78 40 e8  cmp   $e8,#$40    ; |     $0081.2 = 0
236b: b0 06     bcs   $2373       ; | return
236d: f8 e8     mov   x,$e8       ; |     
...
4000: 8d ff     mov   y,#$ff
4002: e8 00     mov   a,#$00
4004: 8f 4d f2  mov		$f2,#$4d  ;\_
4007: c4 f3			mov		$f3,a     ; |
4009: 8f 2c f2	mov		$f2,#$2c  ; | Disable Echo
400c: c4 f3			mov		$f3,a     ; |
400e: 8f 3c f2  mov   $f2,#$3c  ; |_
4011: c4 f3     mov   $f3,a     ;/
4013: 8f 0c f2  mov   $f2,#$0c  ;\ _
4016: c4 f3     mov   $f3,a     ; | Reset Volume
4018: 8f 1c f2  mov   $f2,#$1c  ; |_
401b: c4 f3     mov   $f3,a     ;/
401d: 8f 4c f2  mov   $f2,#$4c  ;\ _
4020: c4 f3     mov   $f3,a     ; | Key On
4022: 8f 5c f2  mov   $f2,#$5c  ; |_Key Off
4025: cb f3     mov   $f3,y     ;/
4027: 3f 0e 41  call  $410e
402a: 8f 5d f2  mov   $f2,#$5d  ;\_ Set the source (sample)
402d: 8f ff f3  mov   $f3,#$ff  ;/  directory to $ff00
4030: 8f 0d f2  mov   $f2,#$0d  ;\_ A whole lotta
4033: 8f 62 f3  mov   $f3,#$62  ;/  echo feedback
4036: 8d 80     mov   y,#$80    ;\ _
4038: cd 00     mov   x,#$00    ; |
403a: e8 00     mov   a,#$00    ; | Clear $0000 - $007f (channel data)
403c: af        mov   (x)+,a    ; |_
403d: fe fd     dbnz  y,$403c   ;/
403f: 8d 40     mov   y,#$40    ;\ _
4041: cd 00     mov   x,#$00    ; |
4043: e8 00     mov   a,#$00    ; |
4045: 40        setp            ; | Clear $0100 - $0140 (?)
4046: af        mov   (x)+,a    ; |
4047: fe fd     dbnz  y,$4046   ; |_
4049: 20        clrp            ;/
404a: 8d 08     mov   y,#$40    ;\ _
404c: cd 88     mov   x,#$88    ; |
404e: e8 01     mov   a,#$01    ; | Set $0088 - $00c7
4050: af        mov   (x)+,a    ; |
4051: 1c        asl   a         ; |_
4052: fe fc     dbnz  y,$4050   ;/ 
4054: e8 00     mov   a,#$00    ;\ _
4056: c4 80     mov   $80,a     ; |
4058: c4 81     mov   $81,a     ; |
405a: c4 83     mov   $83,a     ; |
405c: c4 a0     mov   $a0,a     ; |
...
4064: c4 c5     mov   $c5,a     ; |
4066: c4 c9     mov   $c9,a     ; |
4068: 8f 40 90  mov   $90,#$40  ; |
406b: 8f 40 91  mov   $91,#$40  ; |
406e: 8f 62 92  mov   $92,#$62  ; | 
4071: 8f 00 99  mov   $99,#$00  ; | 
4074: 8f ff 9a  mov   $9a,#$ff  ; | Clear / Set a bunch of memory
4077: 8f 18 9b  mov   $9b,#$18  ; | 
407a: c4 9c     mov   $9c,a     ; | 
407c: c4 9d     mov   $9d,a     ; |
407e: c4 9e     mov   $9e,a     ; |
4080: 8f 18 ce  mov   $ce,#$18  ; |
4083: 8f ff cf  mov   $cf,#$ff  ; |
4086: c4 a1     mov   $a1,a     ; |
4088: 8f ff b1  mov   $b1,#$ff  ; |
408b: 8f ff b0  mov   $b0,#$ff  ; |
408e: 8f 04 95  mov   $95,#$04  ; |
4091: 8f 03 96  mov   $96,#$03  ; |_
4094: 8f 05 97  mov   $97,#$05  ;/
4097: 3f 0d 23  call  $230d     
409a: 8d 93     mov   y,#$93    ;\ _
409c: e8 71     mov   a,#$71    ; |
409e: da a4     movw  $a4,ya    ; |
40a0: 8d 57     mov   y,#$57    ; |
40a2: e8 13     mov   a,#$13    ; | Clear/set even more memory
40a4: da a6     movw  $a6,ya    ; |
40a6: e8 10     mov   a,#$10    ; |
40a8: c5 40 01  mov   $0140,a   ; |
40ab: c5 41 01  mov   $0141,a   ; |_
40ae: 8f 10 e1  mov   $e1,#$10  ;/
40b1: 8f a0 e2  mov   $e2,#$a0  ;\ _
40b4: 8f 01 e3  mov   $e3,#$01  ; |
40b7: 8f 00 e4  mov   $e4,#$00  ; |
40ba: 8f 02 e5  mov   $e5,#$02  ; |
40bd: 8d 00     mov   y,#$00    ; |
40bf: 8f 20 e0  mov   $e0,#$20  ; |
40c2: f7 e2     mov   a,[$e2]+y ; | Transfer data from $01a0..$01bf to \
40c4: d7 e4     mov   [$e4]+y,a ; | $0200..$021f, $0220..$023f, ..., $03e0..$03ff
40c6: fc        inc   y         ; |
40c7: 6e e0 f8  dbnz  $e0,$40c2 ; |
40ca: 60        clrc            ; |
40cb: 98 20 e4  adc   $e4,#$20  ; |
40ce: 98 00 e5  adc   $e5,#$00  ; |_
40d1: 6e e1 e9  dbnz  $e1,$40bd ;/
40d4: e8 00     mov   a,#$00    
40d6: 8f 5c f2  mov   $f2,#$5c  ;\_ Key Off on all the channels
40d9: 8f ff f3  mov   $f3,#$ff  ;/  
40dc: 8f 2d f2  mov   $f2,#$2d  ;\_ Disable Pitch Modulation
40df: c4 f3     mov   $f3,a     ;/  
40e1: 8f 3d f2  mov   $f2,#$3d  ;\_ Disable Noise
40e4: c4 f3     mov   $f3,a     ;/
40e6: 8f 01 93  mov   $93,#$01  ;\_ Set Echo Filters from $13bf
40e9: 3f 9a 13  call  $139a     ;/
40ec: 8f 02 9f  mov   $9f,#$02  ;\_ Set Echo Volumes to #$e0 and #$20
40ef: 3f 68 13  call `$1368     ;/  for left and right, respectively
40f2: 8f 0c f2  mov   $f2,#$0c  ;\ _
40f5: 8f 7f f3  mov   $f3,#$7f  ; | Maximum
40f8: 8f 1c f2  mov   $f2,#$1c  ; |_main volume
40fb: 8f 7f f3  mov   $f3,#$7f  ;/
40fe: 8f 00 f1  mov   $f1,#$00  ;>- Disable the timers
4101: 8f a7 fa  mov   $fa,#$a7  ;>- Set Timer 0 to count every 21 ms
4104: 8f 7d fb  mov   $fb,#$7d  ;>- Set Timer 1 to count every 15.75 ms
4107: 8f 01 fc  mov   $fc,#$01  ;>- Set Timer 2 to count every 31.25 mcs
410a: 8f 07 f1  mov   $f1,#$07  ;>- Enable all timers
410d: 6f        ret

410e: 8f 28 e2  mov   $e2,#$28  ;\ _
4111: 8f d7 e1  mov   $e1,#$d7  ; | 
4114: e8 00     mov   a,#$00    ; |     
4116: c4 e0     mov   $e0,a     ; | Clear the 
4118: fd        mov   y,a       ; | echo buffer
4119: d7 e0     mov   [$e0]+y,a ; | and more
411b: fc        inc   y         ; | ($D700 - $FE00)
411c: d0 fb     bne   $4119     ; |
411e: ab e1     inc   $e1+x     ; |_
4120: 6e e2 f6  dbnz  $e2,$4119 ;/
4123: 8f 00 f1  mov   $f1,#$00  ;\ _
4126: 8f 82 fa  mov   $fa,#$82  ; |_Set up Timer 0 to count every 15.75 ms
4129: 8f 01 f1  mov   $f1,#$01  ;/
412c: 8f 7d f2  mov   $f2,#$7d  ;\ _
412f: eb f3     mov   y,$f3     ; | 
4131: ad 0f     cmp   y,#$0f    ; |
4133: 90 02     bcc   $4136     ; | Wait until Timer 0 has
4135: 8d 0f     mov   y,#$0f    ; | counted {echo delay} times (0..260 ms)
4136: fc        inc   y         ; | (probably to finish clearing the echo buffer)
4138: e4 fd     mov   a,$fd     ; |
413a: f0 fc     beq   $4138     ; |_
413c: fe fa     dbnz  y,$4138   ;/
413e: 8f 6c f2  mov   $f2,#$6c  ;\_ Write to FLG (Noise clock - 0F, Disable Echo)
4141: 18 2f f3  or    $f3,#$2f  ;/
4144: 8f 6d f2  mov   $f2,#$6d  ;\_ Set Echo buffer offset to $D700 
4147: 8f d7 f3  mov   $f3,#$d7  ;/
414a: 8f 7d f2  mov   $f2,#$7d  ;\_ Set Echo Delay to #$30 ms / 
414d: 8f 05 f3  mov   $f3,#$f3  ;/  Echo buffer Size to 6 KB ($#1800 bytes, $D700..$EF00)
4150: 8d 10     mov   y,#$10    ;\ _
4152: e4 fd     mov   a,$fd     ; | Wait until Timer 0 has counted
4154: f0 fc     beq   $4152     ; |_#$10 times (260 ms)
4156: fe fa     dbnz  y,$4152   ;/
4158: 8f 6c f2  mov   $f2,#$6c  ;\_ Write to FLG (Noise clock - 1F, Enable Echo, Mute, Reset)
415b: 38 df f3  and   $f3,#$df  ;/
415e: 8d 05     mov   y,#$05    ;\ _
4160: fc        inc   y         ; |  
4161: e4 fd     mov   a,$fd     ; |  Wait until Timer 0 has counted #$06 times (1560 ms)
4163: f0 fc     beq   $4161     ; |_
4165: fe fa     dbnz  y,$4161   ;/
4167: 6f        ret
